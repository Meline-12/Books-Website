...
class SQLInsert(SQLBase):
    ...
    def book(self, value: dict):
        ...
        if self.check.book(value["src_id"]) is None:
            self.cur.execute(
                "INSERT INTO books (title, year, isbn, pages, average_rating, rating_count, publisher_id, language_id, "
    ...
    def author(self, value: dict):
        if self.check.author(value["src_id"]) is None:
            self.cur.execute(
                "INSERT INTO authors (name, books_count, average_rating, rating_count, description, img, src_id) "
                "VALUES(?, ?, ?, ?, ?, ?, ?)", tuple(value.values()))
            self.conn.commit()
